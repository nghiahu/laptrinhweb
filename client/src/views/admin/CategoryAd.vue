<template>
<div>
    <v-breadcrumbs :items="['Admin', 'Danh mục']"></v-breadcrumbs>
    <div class="containerUser">
      <div class="topCategory">
        <button class="btnAppCategory" @click="togleForm"><AnFilledFileAdd/>Thêm danh mục</button>
        <div class="selecDiv">
      ...
      <table class="tableCategory">
        ...
        <tbody v-for="(category,index) in filterCategory" :key="category.id">
          <td>{{ index + 1 + (indexPage - 1) * 4 }}</td>
          <td class="flex justify-center"><img class="w-[10%]" :src="category.image" alt=""></td>
          <td>{{ category.name }}</td>
          <td>{{ category.createAt }}</td>
          <td class="w-[20%]">
            <button class="bg-blue-600 mr-5" @click="handleSeeInfo(category.id)">Thông tin</button>
            <button class="bg-red-600" @click="delCategory(category)">Xóa</button>
          </td>
        </tbody>
      </table>
    ...
    <CreateCategoryForm v-if="isShowFromAdd" @closeForm="togleForm"/>
  </div>
</template>

<script setup>
import { AnFilledFileAdd } from '@kalimahapps/vue-icons';
...
const countPage = computed(() => Math.ceil(categorys.value.length / 4));
const filterCategory = computed(()=> store.state.categoryModule.filterCategory);
...
const delCategory = (category) => {
  ...
  if (result.isConfirmed) {
    ...
    .then(() => {
      if (filterCategory.value.length === 0 && indexPage.value > 1) {
        indexPage.value--; 
      }
      store.dispatch("fetchFilterCategory", { indexPage: indexPage.value, condition: condition.value, inDe: inDe.value, searchValue: searchValue.value });
    });
...
const handleSeeInfo = (iDCategory) =>{
  router.push(`category-detail/${iDCategory}`)
}
 </script>
